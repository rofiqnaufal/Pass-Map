...
for i in range(len(data)):
   if data["Event"][i] == "Pass (successful)":
        ...
        successarrow=plt.arrow(x,y,dx,dy, width=0.5, head_width=2, color="green",alpha=0.02, linestyle="-", label="Successful")
   if data["Event"][i] == "Pass (failed)":
       plt.scatter(data["X"][i], data["Y"][i], color="red", alpha=0.02, s=70)
       x, y = (data["X"][i]), (data["Y"][i])
       dx, dy = (float(data["X2"][i])) - (float((data["X"][i]))), (float(data["Y2"][i])) - (float((data["Y"][i])))
       failedarrow=plt.arrow(x,y,dx,dy, width=0.5, head_width=2, color="red", alpha=0.02, linestyle=":", label="Failed")
   if data["Event"][i] == "Key pass":
       plt.scatter(data["X"][i], data["Y"][i], color="orange", marker="*", alpha=1, s=70)
       x, y = (data["X"][i]), (data["Y"][i])
       dx, dy = (float(data["X2"][i])) - (float((data["X"][i]))), (float(data["Y2"][i])) - (float((data["Y"][i])))
       keyarrow=plt.arrow(x,y,dx,dy, width=0.5, head_width=2, color="orange", alpha=1, linestyle="-", label="Key Pass")
